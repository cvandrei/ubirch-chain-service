...
if (awsAccessKeyId not in os.environ or awsSecretAccessKey not in os.environ):
    ...
    logger.info(
        "SQS_CHAIN_IN -> AWS SQS queue name of inbound data which chain-service subscribes to(optional), default is 'localdev_ubirch_chainservice_device_msg_in'")
...
inQueue = os.environ[sqsChainInKey] if sqsChainInKey in os.environ else "localdev_ubirch_chainservice_device_msg_in"
